...
class Warehouse implements Serializable {
   ...
   */
  Product getProduct(String id) throws UnknownObjectKeyException {
   ...
   * @throws UnknownObjectKeyException
   */
  Partner getPartner(String id) throws UnknownObjectKeyException {
   ...
   * @throws UnknownObjectKeyException
   */
  void addBatch(String idPartner, String idProduct, int stock, double price) throws UnknownObjectKeyException {
  ...
  List<Batch> getBatchesByPartner(String partnerId) throws UnknownObjectKeyException {
    List<Batch> batchList = new ArrayList<>();
  ...
  void checkDerivativeStock(Product product, int amount) throws NotEnoughResourcesException {
  ...
  double createProduct(Product product) throws NotEnoughResourcesException {
    double balance = 0.0;
    for (RecipeComponent component : product.getRecipe()) {
  ...
  void registerPartnerPayment(int transactionId) throws UnknownObjectKeyException {
    Transaction transaction = getTransaction(transactionId);
    _salesBalance += transaction.pay();
  }

  List<Transaction> getTransactionsByPartner(String partnerId, TransactionType type) throws UnknownObjectKeyException {
  ...
  double getTransactionsHighestPrice(String productId) {
    double price = 0.0;
    for (Transaction transaction : _transactions.values()) {
  ...
  Transaction getTransaction(int transactionId) throws UnknownObjectKeyException {
  ...
  double getAccountingBalance() {
    double balance = 0.0;
    for (Transaction transaction : _transactions.values()) {
      //System.out.println(transaction.getProduct().getId() + " x" + transaction.getQuantity() + " - " + transaction.calculatePriceToPay());
  ...
  void increaseAcquisitionsBalance(double price) {
    _acquisitionsBalance += price;
  }

  void increaseSalesBalance(double price) {
    _salesBalance += price;
  }
  ...
  void toggleNotificationStatus(String partnerId, String productId) throws UnknownObjectKeyException {
