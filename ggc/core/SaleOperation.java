        ...
        if (!product.hasAvailableStock(amount)) {
            ...
            if (!product.isSimple()) {
                ...
                try {
                    ...
                    SaleOperation operation = new SaleOperation(getStore(), _paymentDeadline);
                    operation.executeWithoutTransaction(partner.getId(), exception.getObjectKey(), exception.getRequestedAmount());
                }
            } else {
